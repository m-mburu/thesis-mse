...
This thesis uses DHS survey data
(\citeproc{ref-dhs_drc_2007_dataset}{The DRC Program Program, 2007}). On
the DHS privacy page(\citeproc{ref-dhs_privacy_respondents}{The DHS
Program, 2026}), they state that this voluntary survey data is known to
have some limitations, such as non-response bias and recall bias,
because deaths were recorded after the fact. We also used an older data
set because we wanted to compare the new method with a data set with
known results. So the results might not reflect the current status in
the Democratic Republic of Congo.

The second limitation is the socioeconomic ranking variable, which is
reconstructed using PCA
(\citeproc{ref-dhsprogram_wealth_index_construction}{The DHS Program,
2007}). The DHS wealth index is useful for ordering households from
poorest to richest. It is also a relative score within the survey. This
imitation mainly affects the level-dependent concentration index. This
specifically affects \(L\), because the level-dependent index has been
shown to perform best for wealth variables with rational properties
(\citeproc{ref-erreygers2017socioeconomic}{Erreygers and Kessels,
2017}).

We also used decision trees, which are known to be unstable. Small
changes in the data can produce different split variables, split points,
and tree structures. This instability arises from the recursive, greedy
nature of tree construction: once an early split changes, all subsequent
splits are fit to different subsets of the data. Since this is a
concentration index tree, we tried to address this by introducing a
minimum relative gain, so that we can admit splits that remove a set
amount of gain, chosen via cross-validation.
...
(\citeproc{ref-dhs_drc_2007_dataset}{The DRC Program Program, 2007}). On
the DHS privacy page(\citeproc{ref-dhs_privacy_respondents}{The DHS
Program, 2026}), they state that this voluntary survey data has some
limitations, such as non-response bias and recall bias, because deaths
were recorded after the fact. We also used an older data set to compare
the new method with one with known results. So the results might not
reflect the current status in the Democratic Republic of Congo.
...
nature of tree construction: once an early split changes, all subsequent
splits are fit to different subsets of the data. Since this is a
concentration index tree, we addressed this by introducing a minimum
relative gain, so that we can admit splits that remove a set amount of
gain, chosen via cross-validation.
