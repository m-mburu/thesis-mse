...
The optional switches below control whether the expensive cross-validation,
forest, and SHAP sections are executed.
...
report_compact_table(
  data.table(
    item = c(
      "Rows in analysis data",
      "Candidate predictors",
      "Output file",
      "Run tree cross-validation",
      "Run concentration-index forests",
      "Run forest cross-validation",
      "Run predictive-forest SHAP",
      "SHAP evaluation rows",
      "SHAP simulation draws"
    ),
    value = c(
      format(nrow(congo_model_dt), big.mark = ",", scientific = FALSE),
      length(congo_predictors),
      results_object_file,
      runtime_params$run_cv,
      runtime_params$run_forest,
      runtime_params$run_forest_cv,
      runtime_params$run_shap,
      runtime_params$shap_n,
      runtime_params$shap_nsim
    )
  ),
...
This is the core generator step. It either screens a fixed control setting or
performs cross-validation before refitting the chosen tree.

```{r build-tree-objects}
...
rpart_summary_out <- copy(lecturer_rpart_summary)
if ("is_terminal" %in% names(rpart_summary_out)) {
  rpart_summary_out <- rpart_summary_out[is_terminal == TRUE]
}
...
if (length(lecturer_rpart_models)) {
  plot_labels <- c(
  ...
  for (method_name in names(lecturer_rpart_models)) {
    ...
    plot_ci_rpart_fit(
      lecturer_rpart_models[[method_name]],
      main = plot_labels[[method_name]] %||% method_name
    )
  }
}
```

# Optional Forest and SHAP Objects

These objects are switched off by default because they are the most expensive
part of the generator, but the full workflow remains in the same document.
`rineq` does not expose a SHAP decomposition API, so the SHAP section uses a
cross-validated `ranger` model and `ineqTrees::shap_conc_decomp` for the four
reported inequality criteria.

```{r build-forest-objects}
...
```{r check-predictive-model-packages}
...
if (!nrow(ranger_tuning)) {
  report_compact_table(
    data.table(note = "Run this QMD with run_shap: true to tune the predictive ranger with tidymodels."),
...
if (!nrow(as.data.frame(selected_ranger_setting))) {
  report_compact_table(
    data.table(note = "No tuned ranger setting is available because run_shap is false."),
...
if (is.null(shap_ci_decomp)) {
  report_compact_table(
    data.table(note = "Run this QMD with run_shap: true to compute the CI SHAP decomposition."),
...
if (is.null(shap_cig_decomp)) {
  report_compact_table(
    data.table(note = "Run this QMD with run_shap: true to compute the CIg SHAP decomposition."),
...
if (is.null(shap_cic_decomp)) {
  report_compact_table(
    data.table(note = "Run this QMD with run_shap: true to compute the CIc SHAP decomposition."),
...
if (is.null(shap_l_decomp)) {
  report_compact_table(
    data.table(note = "Run this QMD with run_shap: true to compute the L SHAP decomposition."),
...
if (!nrow(forest_importance_out)) {
  forest_importance_out <- data.table(
    note = "Run this QMD with run_forest: true to fit concentration-index forests."
  )
} else if ("variable" %in% names(forest_importance_out)) {
...
# Build Positive-Control Simulation Objects

The generator ends with a simulation-based sanity check that the fitted tree
procedure can recover a planted subgroup signal.

```{r build-simulation-objects}
...
```{r simulation-preview, purl=FALSE}
report_compact_table(
  known_subgroup_validation,
  caption = "Positive-control validation of subgroup recovery"
)
```

```{r simulation-tree-preview, purl=FALSE, fig.width=12, fig.height=8}
