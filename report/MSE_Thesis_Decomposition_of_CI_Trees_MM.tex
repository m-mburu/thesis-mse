...
occupation. The SHAP decomposition was interpreted as an explanation of
the fitted predictive model and gave a less concentrated pattern.
Residence, household wealth, mother's education, skilled birth
attendance, parental occupation, and regional indicators all appeared
across the criteria.

The direct trees added the subgroup part of the analysis. The
rank-dependent trees based on the standard concentration index (\(CI\)),
...
well to the primary sampling unit (PSU)-held-out validation folds. The
level-dependent concentration index (\(L\)) method gave a different
tree, beginning with residence and then splitting by occupation, wealth,
and province. The \(L\) tree and forest retained positive validation
gains. However, because the DHS wealth index is not ratio-scale, this
result was treated as a sensitivity analysis rather than as the final
direct result.

The tree adds subgroup rules to the decomposition. It shows the paths to
groups where under-five deaths were more or less concentrated. These are
weighted descriptive results. They were checked with PSU-held-out
validation and PSU bootstrap checks, but they are not formal
survey-design estimates.
...
2022}). Reductions in under-five mortality are linked to improved child
...
household conditions, province (administrative area), maternal
...
At each node, the tree searches over candidate variables and admissible
split points, then selects the variable and split pair that yields the
largest reduction in the weighted within-node concentration-index
...
2007}). The wealth score is a PCA-derived composite measure of household
living standard based on asset ownership, housing materials, water
access, sanitation facilities, and related household amenities. The
continuous wealth score is used to rank children from poorer to richer
households when computing the concentration index. Wealth defines the
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-concentration-curve-1.pdf}}
...
(\citeproc{ref-dhsprogram_wealth_index_construction}{The DHS Program,
2007}). For the \(L\) index, the level-dependent construction is
acceptable only when the socioeconomic variable has ratio-scale
properties (\citeproc{ref-erreygers2017socioeconomic}{Erreygers and
Kessels, 2017}); since the wealth index is a PCA score rather than
income or consumption, results based on \(L\) should therefore be
interpreted with caution. When the concentration indices are used as
impurity measures, the tree may not form the same subgroups under all
concentration indices. A split chosen under \(CI\) may differ from one
chosen under \(CIg\), \(CIc\), or \(L\), because each index defines
inequality in a slightly different way.
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-dhs-cart-regions-1.pdf}}
...
In usual CART (Classification and Regression Trees)
(\citeproc{ref-breiman1984classification}{Breiman \emph{et al.}, 1984}),
splits are chosen mainly for prediction. A regression tree searches for
splits that reduce within-node outcome variation, while a classification
tree searches for splits that improve class separation
(\citeproc{ref-hastie2009elements}{Hastie, Tibshirani and Friedman,
2017}).
...
ranks candidate splits. Let
\(A_t = |2\operatorname{cov}_w(Y_i, R_{i|t})|\) denote the absolute
generalised concentration-index component in node \(t\). Under the
standard concentration index rule, the child-node impurity is relative
to the child-node mean
...
socioeconomic position. Because the tree is an algorithmic partitioning
method rather than a parametric regression model, I assess it by the
apparent impurity reduction and by how well the fitted rules perform on
held-out data (\citeproc{ref-breiman2001two_cultures}{Breiman, 2001}).
In an ideal terminal node, the selected inequality index would be close
to zero. Model selection, therefore, evaluates how much of the root-node
impurity is removed after partitioning, and how much impurity remains
inside the terminal nodes.
...
where \(c\) is the tuning setting being evaluated, \(v\) is the
...
decomposition, since the current decomposition methods rely on GLMs
(\citeproc{ref-wagstaff2003decomposing}{{Wagstaff, van Doorslaer and
Watanabe}, 2003}; \citeproc{ref-speybroeck2010decomposing}{Speybroeck
...
This is identical to the Wagstaff contribution formula
(\citeproc{ref-wagstaff2003decomposing}{{Wagstaff, van Doorslaer and
Watanabe}, 2003}; \citeproc{ref-speybroeck2010decomposing}{Speybroeck
\emph{et al.}, 2010}). This means the SHAP-based decomposition
generalises the standard approach, retaining equivalence under linearity
while expanding to accommodate tree ensembles.
...
rather than as a calibrated probability model. We then used the fastshap
R package to estimate SHAP contributions from the fitted forest
(\citeproc{ref-fastshap2024}{Greenwell, 2024}). The SHAP analysis was
used to describe how the fitted forest used the predictors, not to
estimate causal or population-level contributions of predictors to
under-five mortality.
...
\hyperref[sec-stopping-rules]{tree stopping rules section}. In the rpart
comparison, split search and stopping were controlled via rpart
controls, primarily minsplit, minbucket, and the complexity parameter
cp.~

For the concentration-index trees and forests, validation folds were
created at the DHS primary sampling unit (PSU) level. All children from
the same PSU were kept in the same fold. The model was fitted on the
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-results-wealth-distribution-1.pdf}}
...
agricultural occupation, household wealth and Kinshasa. The fitted
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-ci-tree-cic-1.pdf}}
...
around maternal education and father's agricultural occupation. The full
fitted tree also includes a short preceding birth-interval split among
rural children in high-wealth households, so birth interval appears as a
lower-branch refinement rather than as the main organising feature of
the \(L\) tree.
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-tree-validation-path-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-tree-complexity-path-1.pdf}}
...
\begin{longtable}[t]{>{\raggedright\arraybackslash}p{1.6cm}>{\raggedright\arraybackslash}p{1.6cm}>{\raggedright\arraybackslash}p{1.8cm}>{\raggedright\arraybackslash}p{4.0cm}>{\raggedright\arraybackslash}p{2.7cm}>{\raggedright\arraybackslash}p{2.7cm}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-lecturer-rpart-robust-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-forest-validation-path-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-results-surrogate-tree-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-results-selected-model-variable-importance-1.pdf}}
...
independent, since the tree uses concentration index as an impurity
measure, which measures how socioeconomic ranking and outcome covary.
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-results-simulation-null-trees-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-results-simulation-null-trees-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-results-simulation-null-trees-3.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-results-simulation-null-trees-4.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-results-simulation-tree-1.pdf}}
...
We see the same pattern across the appendix sensitivity plots for the
trees developed using \(CI\), \(CIg\), and \(L\) as impurity measures
(Figure~\ref{fig-appendix-simulation-positive-control-ci};
Figure~\ref{fig-appendix-simulation-positive-control-cig};
...
This thesis developed a tree-based method for analysing socioeconomic
inequality in under-five mortality using the concentration index
impurity. In the DRC data, under-five mortality inequality was not
solely a function of household wealth. Residence, maternal education,
skilled birth attendance, parental occupation, household wealth, and
province all appeared in the results. Overall, 10.32\% of children died
before age five. Deaths were more common among children from low-wealth
households, rural households, mothers with no education, births without
skilled attendance, and in some provinces, especially Katanga. A similar
pattern appears in earlier work. (\citeproc{ref-kandala2014drc}{Kandala
\emph{et al.}, 2014}) reported geographic differences in under-five
mortality in the DRC, while (\citeproc{ref-vanmalderen2013under5}{Van
Malderen, Van Oyen and Speybroeck, 2013}) and
(\citeproc{ref-vanmalderen2019socioeconomic}{Van Malderen \emph{et al.},
2019}) reported wealth, education, residence, and occupation in
decomposition studies of under-five mortality inequality in African
settings.

The indirect methods gave the variable-level results. In the regression
decomposition, mother's education had the largest contribution, followed
by father's agricultural occupation, mother's agricultural occupation,
household wealth, residence, and skilled birth attendance. The SHAP
decomposition from the fitted random forest was less concentrated on one
or two variables. Residence was the largest SHAP contributor, while
household wealth, parental occupation, mother's education, skilled birth
attendance, and some regional indicators also appeared across criteria.
These SHAP results explain the fitted predictive model. They should not
be read as causal effects or as population-level contributions to
under-five mortality.

The direct concentration-index trees gave the subgroup results. The
\(CI\), \(CIg\), and \(CIc\) trees mainly split through mother's
education, father's agricultural occupation, skilled birth attendance,
household wealth, and Katanga. Mortality was lower among children whose
mothers had some education and whose fathers were not in agriculture. It
was higher among children whose mothers had no education, who lived in
low-wealth households, and who resided in Katanga. These branches are
easier to read than a table of percentage contributions because they
show how variables appeared together in the fitted data. The
rank-dependent trees fitted the training data much better than the
held-out PSUs. In training, the \(CI\) tree removed 83.3\% of the
root-node impurity, while the \(CIc\) and \(CIg\) trees removed 81.6\%.
In validation, the gains were negative. This means that the same
branches did not reduce concentration-index impurity when applied to
PSUs that were not used to fit the tree.

The \(L\) tree gave a different result. It was split first by residence,
then by father's agricultural occupation, household wealth, birth
interval, and province. This structure had positive validation gain,
unlike the rank-dependent trees. This made the \(L\) result more stable
in this application. It should still be read as a sensitivity result
because the DHS wealth index is a PCA-derived asset score, not a
ratio-scale measure such as income, consumption, or expenditure.
...
criterion, re-ranking is still done. However, in \(L\), the child's
observed socioeconomic level is carried into the subgroup, so a child's
rank also reflects their wealth level.

The differences also showed up in cross-validation; the level-dependent
method had stronger validation performance than the rank-dependent
methods. The rank-dependent trees had strong training performance but
negative validation gains, while the level-dependent tree kept a
positive validation gain
(Figure~\ref{fig-appendix-tree-validation-path};
Table~\ref{tbl-appendix-tree-selected-setting-summary}). The forest
results were directionally similar: averaging trees reduced the
validation loss for the rank-dependent criteria, but their
validation-gain ratios remained below zero, whereas the \(L\) forest
retained a positive validation gain
(Figure~\ref{fig-appendix-forest-validation-path};
Table~\ref{tbl-appendix-forest-selected-setting-summary}). The
validation results show that \(CI\), \(CIg\), and \(CIc\) identified
detailed in-sample subgroup rules, whereas \(L\) produced the more
reproducible descriptive partition in this application.
...
decompose cleanly across population groups.
(\citeproc{ref-erreygers2017subgroup}{Erreygers \emph{et al.}, 2017})
showed that the level-dependent method is decomposable into subgroups,
and this may also help explain why \(L\) validated better as a tree
impurity criterion in this application: children inherit their
socioeconomic level within each subgroup, rather than being re-weighted
only by a new within-node rank. However, the DHS wealth index is an
asset-based score
(\citeproc{ref-dhsprogram_wealth_index_construction}{The DHS Program,
2007}), and (\citeproc{ref-erreygers2017socioeconomic}{Erreygers and
Kessels, 2017}) recommends using the level-dependent concentration index
(\(L\)) with a socioeconomic variable that has ratio-scale properties.
The cross-validation results, therefore, suggest that \(L\) yielded the
more transferable descriptive partition in this application. However,
the \(L\) result should be treated as a sensitivity analysis rather than
as a final claim about level-dependent inequality.
...
This stems from the recursive, greedy nature of tree construction. Once
an early split changes, all later splits are fitted to different subsets
of the data. In this thesis, this was handled partly through
PSU-held-out cross-validation, bootstrap stability checks, and splitting
controls such as minimum node size, minimum child-node proportion,
maximum depth, minimum gain, and minimum relative gain. These controls
helped restrict weak or very small splits and describe the stability of
the selected trees, but they do not eliminate the instability problem.

The tree models did not use the full DHS survey design as the regression
model did. The trees used DHS sampling weights, and the validation and
bootstrap checks were done at the PSU level. These steps helped with
weighting and with cluster validation of the fitted trees. They,
however, did not produce survey-design standard errors. I therefore do
not report design-based confidence intervals for individual splits,
forest summaries, or SHAP contributions.
...
using the forest's prediction scores. The surrogate still chooses splits
by maximising concentration-index gain, but the outcome used in that
split rule is the forest-predicted score rather than the observed
mortality outcome. This approximation of the forest, therefore, depends
on how well the surrogate tree represents the forest.
...
alive and 40.0\% died. Since prediction scores were not recalibrated,
...
SHAP results are treated as a descriptive explanation of the fitted
predictive model, rather than as evidence that the listed variables
contribute those proportions to the risk of under-five mortality.
...
rank-dependent trees. The SHAP-based decomposition also needs to be
improved and studied further.
...
occupation. The SHAP-based decomposition provided a broader explanation
of the fitted model, with residence, household wealth, mother's
education, skilled birth attendance, parental occupation, and regional
indicators appearing across the criteria.

The concentration-index trees were used as the direct decomposition
method. They showed the subgroups where under-five deaths remained
concentrated after splitting the data. The \(CI\), \(CIg\), and \(CIc\)
trees mainly separated children by mother's education, father's
agricultural occupation, household wealth, skilled birth attendance, and
Katanga. The higher-mortality branch was children whose mothers had no
education, who lived in low-wealth households, and who resided in
Katanga. These trees fitted the training data, but their PSU-held-out
validation gains were negative. The \(L\) tree was different. It was
first split by residence, then by occupation, household wealth, and
province.
...
not transfer well to held-out PSUs. The \(L\) tree and forest retained
positive validation gains, so the level-dependent result was the more
stable descriptive partition in this application. However, because the
DHS wealth index is not a ratio-scale socioeconomic variable, the \(L\)
result should be interpreted as a sensitivity analysis rather than as a
final level-dependent inequality claim.
...
\bibitem[\citeproctext]{ref-breiman2001two_cultures}
Breiman, L. (2001) {`Statistical modeling: The two cultures'},
\emph{Statistical Science}, 16(3), pp. 199--231. Available at:
...
\bibitem[\citeproctext]{ref-fastshap2024}
Greenwell, B. (2024) \emph{Fastshap: Fast approximate shapley values}.
Available at: \url{https://doi.org/10.32614/CRAN.package.fastshap}.
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-province-mortality-distribution-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-shap-summary-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-tree-variable-importance-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-ci-tree-cig-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-ci-tree-ci-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-lecturer-rpart-simple-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-forest-complexity-path-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-forest-variable-importance-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-forest-surrogate-ci-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-forest-surrogate-cig-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-forest-surrogate-cic-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-forest-surrogate-l-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-simulation-positive-control-ci-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-simulation-positive-control-cig-1.pdf}}

}

\caption{\label{fig-appendix-simulation-positive-control-cig}Positive-control
simulation tree fitted with the CIg criterion.}
...
\pandocbounded{\includegraphics[keepaspectratio]{MSE_Thesis_Decomposition_of_CI_Trees_MM_files/figure-pdf/fig-appendix-simulation-positive-control-l-1.pdf}}
